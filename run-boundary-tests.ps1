...
$compileLog = "bin\javac-boundary-stderr.log"
...
if ($LASTEXITCODE -ne 0 -and -not (Test-Path "bin\test\BoundaryValueTestRunner.class")) {
    if (Test-Path $compileLog) {
...
java -cp $runClasspath test.BoundaryValueTestRunner
exit $LASTEXITCODE

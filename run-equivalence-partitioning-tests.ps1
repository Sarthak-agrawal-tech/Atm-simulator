...
$compileLog = "bin\javac-equivalence-partitioning-stderr.log"
...
if ($LASTEXITCODE -ne 0 -and -not (Test-Path "bin\test\EquivalencePartitioningTestRunner.class")) {
    if (Test-Path $compileLog) {
...
java -cp $runClasspath test.EquivalencePartitioningTestRunner
exit $LASTEXITCODE

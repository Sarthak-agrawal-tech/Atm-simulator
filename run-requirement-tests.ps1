...
$compileLog = "bin\javac-requirement-stderr.log"
...
if ($LASTEXITCODE -ne 0 -and -not (Test-Path "bin\test\RequirementBasedTestRunner.class")) {
    if (Test-Path $compileLog) {
...
java -cp $runClasspath test.RequirementBasedTestRunner
exit $LASTEXITCODE

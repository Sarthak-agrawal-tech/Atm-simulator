...
$compileLog = "bin\javac-decision-table-stderr.log"
...
if ($LASTEXITCODE -ne 0 -and -not (Test-Path "bin\test\DecisionTableTestRunner.class")) {
    if (Test-Path $compileLog) {
...
java -cp $runClasspath test.DecisionTableTestRunner
exit $LASTEXITCODE
